...
var DEFAULT_BUILDING_SITE_SAAS_UUID = "e144ad6c-e8ba-4bf9-8157-80e38af529e7";
    ...
    "properties": {
      "eventTag": {
        ...
        "enum": [
          ...
          "EVENT_SMART_CONSTRUCTION_EMPLOYEE_ACCESS_REPORT"
        ]
      },
      "eventData": {
        "type": "object",
    ...
    if (error) {
      ...
      async.waterfall([
          function (innerCallback) {
            logger.debug(message.eventData.uuid);
          ...
          function (deviceInfo, innerCallback) {
            ...
            else if (message.eventTag === "EVENT_SMART_CONSTRUCTION_EMPLOYEE_ACCESS_REPORT") {
              event.eventDescription = "Employ " + event.deviceName + " access.";
              logger.debug(event);
              self.message({
                devices: DEFAULT_BUILDING_SITE_SAAS_UUID,
                payload: {
                  cmdName: "sync_access_records",
                  cmdCode: "0002",
                  parameters: {
                    "recordId": uuid.v4(),
                    "recordTime": moment(Date.now()).format('YYYY-MM-DD HH:mm:ss'),
                    "userId": message.eventData.uuid,
                    "state": message.eventData.state,
                    "deviceId": message.eventData.deviceId,
                    "deviceName":message.eventData.deviceName,
                    "recordType": message.eventData.recordType
                  }
                }
              }, function (response) {
                if (response.retCode !== 200) {
